...
def _get_redis():
    ...
    dbindex = odoo.tools.config.get_misc('redis_config', 'redis_dbindex', None)
    redis_instance = redis.Redis(host=host, port=port, password=password, db=dbindex)

    if sentinel_host:
        sentinel = Sentinel([(sentinel_host, sentinel_port)], password=password, db=dbindex)
...
class RedisSessionStore(SessionStore):
    ...
    """
    def __init__(self, prefix='session:', session_class=None):
    ...
    def get(self, session_id):
        ...
        if not saved:
            ...
            return self.new()
        try:
            data = yaml.safe_load(saved.decode("utf-8"))
        except ValueError:
            _logger.debug(
